...
export default function RNApp() {
    ...
    const [headerListHeight, setHeight] = useState(null);
                    ...
                    renderItem={({ item, index }) => {
                        ...
                        return (
                            <TouchableOpacity style={styles.titleView} onPress={() => {
                                console.log(dataSourceCords, index, headerListHeight);
                                // if (index == 0) {
                                //   let scrollObj = {
                                //     x: 0,
                                //     // y: dataSourceCords[index] - (headerListHeight / (index + 2) - 10),
                                //     y: dataSourceCords[index] - headerListHeight / 2 - 10,
                                //     animated: true,
                                //   }
                                //   console.log("scrollObj--index0->", scrollObj)
    ...
    function renderHeaderListItem({ item, index }) {
        return (
            <View style={{ marginTop: 10, borderWidth: 0.5, borderColor: "gray" }}>
                <Text style={styles.titleStyle}>{item.title}</Text>
            ...
            >
                <View style={styles.topView}>
                    ...
                    <FlatList
                        ...
                        onLayout={(event) => {
                            const layout = event.nativeEvent.layout;
                            console.log('layout: header List', layout);
                            setHeight(layout.y)
                        }}
                        bounces={false}
                        showsVerticalScrollIndicator={false}
